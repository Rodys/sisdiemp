	...
	<center>
		...
		<div class="box">
			...
			if($nome == $aluno || isset($id) ){	
				if(isset($id)){
					$sql = "select id,Nome,Matricula as 'Matrícula',Unidade,CPF, telefone, endereco, email, data_nasc from aluno where id = '$id' ORDER BY nome";
				}
				else{
					$sql = "select id,Nome,Matricula as 'Matrícula',Unidade,CPF, telefone, endereco, email, data_nasc from aluno where nome like '%$aluno%' ORDER BY nome";
				}
			...
			if(($nome != $aluno)||($tmp == true)||(($cpf == '')&&(isset($id)))){
				if((($cpf == '')&&(isset($id)))){
					...
					$sql="select id, nome, cpf, matricula, curso, unidade, telefone, endereco, email, data_nasc as nascimento from aluno where id = '$id'";
				}
				$res = mysqli_query($c, $sql);
				$line = mysqli_fetch_assoc($res);
